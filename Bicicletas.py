...
#Clase para gestionar las bicicletas
class GestionBicicletas:
    ventana_actual = None
    
#Metodo para cerrar la ventana anterior
    @staticmethod
...
#Metodo para obtener las marcas de las bicicletas
    @staticmethod
    def obtener_marcas():
...
#Metodo para modificar
    ...
    def modificar_bicicleta():
        ...
        def buscar_serial():
            ...
            ctk.CTkButton(ventana, text="Guardar", command=guardar_cambio, fg_color="#5cb85c", font=("Arial", 14), width=100).pack(pady=5) 
...
#Metodo para buscar
    ...
    def buscar_bicicletas():
        ...
        ctk.CTkLabel(buscar_frame, text="Ingrese el serial de la bicicleta:", font=("Arial", 12)).pack(pady=5, padx=10)
        entrada_serial = ctk.CTkEntry(buscar_frame, width=200)
        entrada_serial.pack(pady=5, padx=10)
        ...
        def buscar():
            ...
            serial = entrada_serial.get().strip()
            if not serial:
                resultado_label.configure(text="Ingrese un serial.", text_color="red")
                return

            try:
                serial = int(serial)
            except ValueError:
                resultado_label.configure(text="El serial debe ser un número entero.", text_color="red")
                return
            ...
            if not resultado:
                resultado_label.configure(text="Bicicleta no encontrada.", text_color="red")
                if scroll_frame.winfo_exists():
                    ctk.CTkLabel(scroll_frame, text="No se encontró la bicicleta.", font=("Arial", 12)).pack(pady=10)
            else:
                resultado_label.configure(text="Bicicleta encontrada.", text_color="green")
                if scroll_frame.winfo_exists():
                    frame = ctk.CTkFrame(scroll_frame, fg_color="#3c3c3c", corner_radius=5)
                    frame.pack(pady=5, fill="x")

                    ctk.CTkLabel(frame, text="Información de la bicicleta:", font=("Arial", 12, "bold")).grid(row=0, column=0, columnspan=2, padx=10, pady=5)

                    ctk.CTkLabel(frame, text="Serial:", font=("Arial", 11)).grid(row=1, column=0, padx=10, pady=5)
                    ctk.CTkLabel(frame, text=str(resultado[0]), font=("Arial", 11)).grid(row=1, column=1, padx=10, pady=5)

                    ctk.CTkLabel(frame, text="Marca:", font=("Arial", 11)).grid(row=2, column=0, padx=10, pady=5)
                    ctk.CTkLabel(frame, text=resultado[1], font=("Arial", 11)).grid(row=2, column=1, padx=10, pady=5)

                    ctk.CTkLabel(frame, text="Color:", font=("Arial", 11)).grid(row=3, column=0, padx=10, pady=5)
                    ctk.CTkLabel(frame, text=resultado[2], font=("Arial", 11)).grid(row=3, column=1, padx=10, pady=5)

                    ctk.CTkLabel(frame, text="Modelo:", font=("Arial", 11)).grid(row=4, column=0, padx=10, pady=5)
                    ctk.CTkLabel(frame, text=str(resultado[3]), font=("Arial", 11)).grid(row=4, column=1, padx=10, pady=5)

                    ctk.CTkLabel(frame, text="Referencia:", font=("Arial", 11)).grid(row=5, column=0, padx=10, pady=5)
                    ctk.CTkLabel(frame, text=resultado[4], font=("Arial", 11)).grid(row=5, column=1, padx=10, pady=5)

                    ctk.CTkLabel(frame, text="Tamaño de la llanta:", font=("Arial", 11)).grid(row=6, column=0, padx=10, pady=5)
                    ctk.CTkLabel(frame, text=str(resultado[5]), font=("Arial", 11)).grid(row=6, column=1, padx=10, pady=5)

                    ctk.CTkLabel(frame, text="Tipo:", font=("Arial", 11)).grid(row=7, column=0, padx=10, pady=5)
                    ctk.CTkLabel(frame, text=resultado[6], font=("Arial", 11)).grid(row=7, column=1, padx=10, pady=5)

                    ctk.CTkLabel(frame, text="Estado:", font=("Arial", 11)).grid(row=8, column=0, padx=10, pady=5)
                    ctk.CTkLabel(frame, text=resultado[7], font=("Arial", 11)).grid(row=8, column=1, padx=10, pady=5)
